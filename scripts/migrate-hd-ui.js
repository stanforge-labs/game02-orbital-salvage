...
setInstance(I.cargo, { x: 82, y: 88, width: 340, height: 42, customSize: true });
setInstance(I.hull, { x: 82, y: 151, width: 340, height: 42, customSize: true });
setInstance(I.credits, { x: 1450, y: 88, width: 380, height: 42, customSize: true });
setInstance(I.sector, { x: 1450, y: 132, width: 380, height: 70, customSize: true });
...
for (const action of actions) {
  ...
  if (code.includes('__osInit')) code = code.replace("v.get('Credits').setNumber(0);", "v.get('Credits').setNumber(0);if(typeof window!=='undefined'&&window.location.hostname==='127.0.0.1'){const q=new URLSearchParams(window.location.search);if(q.has('qaCredits'))v.get('Credits').setNumber(Number(q.get('qaCredits'))||0);if(q.has('qaState'))v.get('GameState').setString(q.get('qaState'));if(q.has('qaCargoMax'))v.get('CargoMax').setNumber(Number(q.get('qaCargoMax'))||8);if(q.has('qaHullMax')){v.get('HullMax').setNumber(Number(q.get('qaHullMax'))||3);v.get('Hull').setNumber(v.get('HullMax').getAsNumber());}if(q.has('qaEngine'))v.get('EngineLevel').setNumber(Number(q.get('qaEngine'))||0);}");
