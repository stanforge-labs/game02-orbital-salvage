...
async function open(viewport={width:1920,height:1080},mobile=false){
 const browser=await chromium.launch({headless:process.env.YANDEX_QA_HEADED!=='1',executablePath}),context=await browser.newContext({viewport,isMobile:mobile,hasTouch:mobile}),p=await context.newPage(),errors=[],external=[];
