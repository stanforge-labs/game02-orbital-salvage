...
const tracePattern = /if\(typeof window!=='undefined'&&window\.location\.search\.includes\('qaTrace'\)\)\{window\.__osTrace=window\.__osTrace\|\|\[\];window\.__osTrace\.push\(\{t:performance\.now\(\),shipX:s\.x,shipY:s\.y,cameraX:cam\.x,cameraY:cam\.y,speed:Math\.hypot\(s\.vx,s\.vy\)\}\);if\(window\.__osTrace\.length>\d+\)window\.__osTrace\.shift\(\);\}/g;
let changed = 0;
function visit(value) {
  ...
  if (value.type === 'BuiltinCommonInstructions::JsCode' && Array.isArray(value.inlineCode)) {
    ...
    code = code.replace(tracePattern, '');
    code = code.replace(/if\(typeof window!=='undefined'\)window\.__osQADebug=\{.*?\};/g, '');
