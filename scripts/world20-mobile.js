module.exports=function mobile20(scene){
 ...
 if(window.__mobile20Child){
  ...
  window.__mobile20Telemetry=()=>{const now=performance.now();if(now-last>500){fps=frames*1000/(now-last);last=now;frames=0;}const v=scene.getVariables(),W=innerWidth,H=innerHeight;return {width:W,height:H,logical:[scene.getGame().getGameResolutionWidth(),scene.getGame().getGameResolutionHeight()],hudCamera:[scene.getLayer('HUD').getCameraX(),scene.getLayer('HUD').getCameraY()],orientation:H>W?'portrait':'landscape',dpr:devicePixelRatio,uiScale:H/1080,fps:Math.round(fps),instances:scene.getAdhocListOfAllInstances().length,input:window.__mobile20Touch?'touch simulation':'mouse / keyboard',state:v.get('GameState').getAsString(),safeInsets:scene.__mobile21?[scene.__mobile21.safe.top,scene.__mobile21.safe.right,scene.__mobile21.safe.bottom,scene.__mobile21.safe.left]:[24,16,24,16],save:'isolated in-memory copy'};};
  const step=()=>{frames++;requestAnimationFrame(step);const now=performance.now();if(now-lastGuide<100)return;lastGuide=now;const c=canvas.getContext('2d'),W=innerWidth,H=innerHeight,opt=window.__mobile20State;if(canvas.width!==W||canvas.height!==H){canvas.width=W;canvas.height=H;}c.clearRect(0,0,W,H);c.lineWidth=1;
   if(opt.safe){c.strokeStyle='#50d0b0';const s=scene.__mobile21?.safe||{left:16,right:16,top:24,bottom:24};c.strokeRect(s.left,s.top,W-s.left-s.right,H-s.top-s.bottom);}
 ...
 launch.onclick=async()=>{
  ...
  let width=390,height=844,disposed=false,timer,browserBars=0;
  ...
  const layout=()=>{const r=stage.getBoundingClientRect(),usableHeight=Math.max(220,height-browserBars),scale=Math.min((r.width-24)/width,(r.height-24)/usableHeight,1);iframe.style.width=width+'px';iframe.style.height=usableHeight+'px';iframe.style.transform=`scale(${scale})`;iframe.style.left=(r.width-width*scale)/2+'px';iframe.style.top=(r.height-usableHeight*scale)/2+'px';};
  ...
  add('Portrait',()=>{[width,height]=[Math.min(width,height),Math.max(width,height)];layout();});add('Landscape',()=>{[width,height]=[Math.max(width,height),Math.min(width,height)];layout();});add('Rotate',()=>{[width,height]=[height,width];layout();});add('Reset viewport',()=>{select.value='390x844';width=390;height=844;browserBars=0;bars.value='0';notch.value='none';notch.onchange();layout();});
  const bars=document.createElement('select');bars.setAttribute('aria-label','Browser UI height');for(const [value,label]of [[0,'Browser UI: hidden'],[56,'Browser UI: 56 px'],[96,'Browser UI: 96 px']]){const o=document.createElement('option');o.value=value;o.textContent=label;bars.appendChild(o);}bars.onchange=()=>{browserBars=Number(bars.value);layout();};tools.appendChild(bars);
  const notch=document.createElement('select');notch.setAttribute('aria-label','Safe area simulation');for(const[value,label]of [['none','Safe insets: none'],['notch','Safe insets: notch + home bar']]){const o=document.createElement('option');o.value=value;o.textContent=label;notch.appendChild(o);}notch.onchange=()=>{const w=iframe.contentWindow,e=w.document.documentElement;for(const[k,val]of Object.entries(notch.value==='notch'?{top:8,right:20,bottom:12,left:28}:{top:0,right:0,bottom:0,left:0}))e.style.setProperty('--safe-'+k,val+'px');w.__mobile21Scene?.__mobile21.refresh();};tools.appendChild(notch);
