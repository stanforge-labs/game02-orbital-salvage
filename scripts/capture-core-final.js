...
(async () => {
  const out = 'C:\\Yandex Games\\02 Orbital Salvage\\screenshots\\CorePass03';
  const errors = [];
  ...
  await page.screenshot({ path: `${out}\\03-station-start.png` });
  ...
  await page.screenshot({ path: `${out}\\04-safe-zone.png` });
  await move(['ArrowRight'], 2000);
  await page.screenshot({ path: `${out}\\05-debris-field.png` });
  await move(['ArrowDown'], 1000);
  await page.screenshot({ path: `${out}\\06-danger-zone.png` });
  await move(['ArrowRight'], 2400);
  await move(['ArrowUp'], 1800);
  await page.screenshot({ path: `${out}\\07-rare-container.png` });
  ...
  await mobilePage.screenshot({ path: `${out}\\10-mobile-landscape.png` });
