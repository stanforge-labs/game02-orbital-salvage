...
module.exports = function applySmart08(layout) {
 ...
 play=replace(play,'const scraps=runtimeScene', `const pickup=(value,label)=>{const n=v.get('Streak').getAsNumber()+1;v.get('Streak').setNumber(n);const bonus=n===3?5:n===5?10:0;if(bonus){v.get('CargoValue').setNumber(v.get('CargoValue').getAsNumber()+bonus);v.get('StreakBonus').setNumber(v.get('StreakBonus').getAsNumber()+bonus);}s.floatText='+'+value+(bonus?'  СЕРИЯ '+n+'  +'+bonus:'');s.floatLife=1.4;s.toast=1.4;v.get('Status').setString(label+(bonus?' • СЕРИЯ '+n+' +'+bonus:''));};\nconst scraps=runtimeScene`);
 ...
 play=play.split("v.get('RunDamage').setNumber(v.get('RunDamage').getAsNumber()+1);").join("v.get('RunDamage').setNumber(v.get('RunDamage').getAsNumber()+1);v.get('Streak').setNumber(0);s.floatText='−1 КОРПУС';s.floatLife=1.1;");
 ...
 play+='\nconst floating=runtimeScene.getObjects("PickupText")[0];s.floatLife=Math.max(0,(s.floatLife||0)-dt);if(floating){floating.hide(s.floatLife<=0);if(s.floatLife>0){floating.setString(s.floatText||"");floating.setPosition(s.x-110,s.y-60-(1.4-s.floatLife)*20);floating.setOpacity(Math.min(255,s.floatLife*350));}}';
